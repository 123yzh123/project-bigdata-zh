...
WITH detail AS (SELECT user_id,
     ...
     sku AS (SELECT id,
             ...
             FROM gmall.dim_sku_full
             WHERE dt = '2024-09-18')
INSERT OVERWRITE TABLE gmall.dws_trade_user_sku_order_1d PARTITION( dt = '2024-09-18')
SELECT user_id,
       id,
       sku_name,
       category1_id,
...
-- （2）每日装载
insert overwrite table dws_trade_user_order_1d partition (dt = '2024-09-18')
select user_id,
       count(distinct (order_id)),
       sum(sku_num),
       sum(split_original_amount),
...
-- （2）每日装载
insert overwrite table dws_trade_user_cart_add_1d partition (dt = '2024-09-18')
select user_id,
       count(*),
       sum(sku_num)
from dwd_trade_cart_add_inc
...
WITH
    ...
    province AS (SELECT id,
                 ...
                 FROM gmall.dim_province_full
                 WHERE dt = '2024-09-18')
INSERT OVERWRITE TABLE gmall.dws_trade_province_order_1d PARTITION(dt = '2024-09-18')
SELECT province_id,
       province_name,
       area_code,
       iso_code,
...
INSERT OVERWRITE TABLE gmall.dws_traffic_session_page_view_1d PARTITION(dt = '2024-09-18')
SELECT
    session_id,
    mid_id,
    brand,
...
SELECT * FROM gmall.dws_traffic_session_page_view_1d WHERE dt='2024-09-18';
...
-- =========================================================================
SET hive.stats.autogather=false;
INSERT OVERWRITE TABLE gmall.dws_traffic_page_visitor_page_view_1d PARTITION(dt = '2024-09-18' )
SELECT
    mid_id,
    brand,
    model,
...
SHOW PARTITIONS gmall.dws_traffic_page_visitor_page_view_1d ;
SELECT * FROM gmall.dws_traffic_page_visitor_page_view_1d WHERE dt = '2024-09-18';
...
*/
INSERT OVERWRITE TABLE gmall.dws_trade_user_sku_order_nd PARTITION (dt = '2024-09-18')
SELECT user_id,
       sku_id,
       sku_name,
       category1_id,
       ...
       tm_name,
       -- 最近7日汇总数据
       sum(if(dt >= date_sub('2024-09-18', 6), order_count_1d, 0))            AS order_count_7d,
       sum(if(dt >= date_sub('2024-09-18', 6), order_num_1d, 0))              AS order_num_7d,
       sum(if(dt >= date_sub('2024-09-18', 6), order_original_amount_1d, 0))  AS order_original_amount_7d,
       sum(if(dt >= date_sub('2024-09-18', 6), activity_reduce_amount_1d, 0)) AS activity_reduce_amount_7d,
       sum(if(dt >= date_sub('2024-09-18', 6), coupon_reduce_amount_1d, 0))   AS coupon_reduce_amount_7d,
       sum(if(dt >= date_sub('2024-09-18', 6), order_total_amount_1d, 0))     AS order_total_amount_7d,
       -- 最近30日汇总数据
...
*/
-- （1）首日装载
INSERT OVERWRITE TABLE gmall.dws_trade_user_order_td partition (dt = '2024-09-18')
SELECT user_id,
...
SELECT nvl(old.user_id, new.user_id)                                                           AS user_id,
       if(new.user_id IS NOT NULL AND old.user_id IS NULL, '2024-09-18', old.order_date_first) AS order_date_first,
       if(new.user_id IS NOT NULL, '2024-09-18', old.order_date_last)                          AS order_date_last,
       ...
       nvl(old.activity_reduce_amount_td, 0) +
       nvl(new.activity_reduce_amount_1d, 0)                                                   AS activity_reduce_amount_td,
       nvl(old.coupon_reduce_amount_td, 0) +
       nvl(new.coupon_reduce_amount_1d, 0)                                                     AS coupon_reduce_amount_td,
...
--（1）首日装载
INSERT OVERWRITE TABLE dws_user_user_login_td PARTITION (dt = '2024-09-18')
SELECT u.id,
...
select nvl(old.user_id, new.user_id),
       if(new.user_id is null, old.login_date_last, '2024-09-18'),
       nvl(old.login_count_td, 0) + nvl(new.login_count_1d, 0)
from (select user_id,
             login_date_last,
             login_count_td
      from dws_user_user_login_td
      where dt = date_add('2024-09-18', -1)) old
         full outer join
     (select user_id,
