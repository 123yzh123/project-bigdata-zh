...
WITH
    ...
    info AS (SELECT id,
             ...
             WHERE dt = '2024-09-11'),
    -- c. 订单明细活动关联表
    activity AS (SELECT order_detail_id,
                        activity_id,
                        activity_rule_id
                 FROM gmall.ods_order_detail_activity_inc
                 WHERE dt = '2024-09-11'),
    -- d. 订单明细优惠卷关联表
    coupon AS (SELECT order_detail_id,
                      coupon_id
...
OVERWRITE
TABLE
gmall.dwd_trade_order_detail_inc
PARTITION
(
dt
)
...
-- （1）首日装载
WITH order_detail AS (SELECT id,
                             order_id,
                             sku_id,
                             source_id,
                             source_type,
                      ...
                      FROM gmall.ods_order_detail_inc
                      WHERE dt = '2024-09-11'),
     payment_info AS (SELECT user_id,
                             order_id,
                             payment_type,
                             callback_time
                      FROM gmall.ods_payment_info_inc
                      WHERE dt = '2024-09-11'),
     order_info AS (SELECT id AS order_id,
                           province_id
                    FROM gmall.ods_order_info_inc
                    WHERE dt = '2024-09-11'),
     order_detail_activity AS (SELECT order_detail_id,
                                      activity_id,
                                      activity_rule_id
                               FROM gmall.ods_order_detail_activity_inc
                               WHERE dt = '2024-09-11'),
     order_detail_coupon AS (SELECT order_detail_id,
                                    coupon_id
                             FROM gmall.ods_order_detail_coupon_inc
                             WHERE dt = '2024-09-11'),
     base_dic AS (SELECT dic_code,
                         dic_name
                  FROM gmall.ods_base_dic_full
                  WHERE dt = '2024-09-12')
INSERT
OVERWRITE
TABLE
gmall.dwd_trade_pay_detail_suc_inc
PARTITION
(
dt
)
...
-- （2）每日装载
SHOW PARTITIONS gmall.dwd_trade_pay_detail_suc_inc;
...
*/
INSERT OVERWRITE TABLE gmall.dwd_trade_cart_full PARTITION (dt = '2024-09-11')
SELECT id,
       user_id,
       sku_id,
       sku_name,
...
-- （2）每日装载
insert overwrite table gmall.dwd_tool_coupon_get_inc partition (dt = '2024-09-12')
select id,
       coupon_id,
       user_id,
       date_format(get_time, 'yyyy-MM-dd') date_id,
...
SHOW PARTITIONS gmall.dwd_tool_coupon_get_inc;
...
OVERWRITE
TABLE
gmall.dwd_interaction_favor_add_inc
PARTITION
(
dt
)
...
FROM favor_info;
-- （2）每日装载
SHOW PARTITIONS gmall.dwd_interaction_favor_add_inc;

-- todo 9 互动域-评价-事务事实表
-- （1）首日装载
WITH comment_info AS (SELECT id,
                             user_id,
                             nick_name,
                             head_img,
                             sku_id,
...
overwrite
table
gmall.dwd_interaction_comment_inc
partition
(
dt
)
...
SHOW PARTITIONS gmall.dwd_user_register_inc;

-- todo 19 用户域-用户登录-事务事实表
WITH
-- 最内层子查询
t1 AS (SELECT common.uid                              AS user_id,
...
t2 AS (SELECT user_id,
              ...
              ts,
              CONCAT(mid_id, '-',
                     LAST_VALUE(session_start_point, TRUE) OVER (PARTITION BY mid_id ORDER BY ts)) AS session_id
       FROM t1),
-- 第三层子查询
...
SHOW PARTITIONS gmall.dwd_user_login_inc;
